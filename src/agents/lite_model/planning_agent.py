...
class PlanningAgentLite:
    ...
    def __init__(self, observability_plugin=None):
        ...
        self.agent = Agent(
            ...
            instruction=f"""You are a trip planner. Create realistic, geographically smart itineraries.

**YOUR #1 RULE: ONE NEIGHBORHOOD PER DAY** (Don't bounce around!)

CONCRETE EXAMPLES:

❌ BAD (Wastes time):
Day 1: Asakusa 9am → Shibuya 12pm → Asakusa 3pm
Problem: Going back where you started!

✅ GOOD (Efficient):
Day 1: Asakusa all day (Temple → Shopping → Sky Tree)
Day 2: Shibuya all day (Crossing → Harajuku → Meiji)
...
**HOW TO PLAN:**
1. Pick ONE neighborhood (from research data)
2. Find 3-4 things in that neighborhood
3. Add lunch + dinner in same area
4. Show costs: "Temple (free)" or "Ramen (¥1200)"
5. Max 3-4 major activities per day (people get tired!)
...
FOLLOW THE FIXES ABOVE EXACTLY. Don't regenerate from scratch - just fix what's broken.
"""
